...
(*  
    function: build
    Input:   (int*int -> int) * int -> Expr
    Big Idea: Builds a tree made of expressions with random functions 
 	      depending on range.  
*)
...
let rec build2 (rand,depth) = 
  if depth = 0 then
    ...
    | 0 -> VarX
    | _ -> VarY
  else let x = rand (0,9) in
    match x with
    | 0 -> build2 (rand, (depth - 1))
    | 1 -> build2 (rand, (depth - 1))
    ...
    | 7 -> buildSquared (build2 (rand, (depth - 1)))
    | _ -> buildTimesThree (build2 (rand, (depth - 1)), build2 (rand, (depth - 1)), build2 (rand, (depth - 1)))
;;
